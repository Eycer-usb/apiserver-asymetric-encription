...
* **Single-Producer, Multiple-Consumer (SPSC) Queue:** Each I/O thread has its own private pool of worker threads. Tasks are dispatched via async with a task queue shared by all the worker threads in the private pool. With the observability APIs you can monitor how many pending tasks are in the shared queue, a vital load indicator. 
...
* **Built-in Observability:** Comes with out-of-the-box monitoring through a set of internal API endpoints for JSON consumers and Prometheus, providing real-time insights into the server's health and performance. Logs are printed in JSON to stderr and stdout with distinct levels, ready to be ingested into Grafana Loki.
* **Powerful abstractions:** Very easy to use and efficient abstractions to create Web APIs that execute database stored procedures via ODBC API or invoke REST APIs via HTTPS with libcurl, with integrated JSON Web Token stateless security model and built-in multifactor authentication via TOPT (Google Authenticator).
...
cd apiserver2 && \
sudo apt install -y g++-14 make libssl-dev libjson-c-dev unixodbc-dev tdsodbc libcurl4-openssl-dev libcrypto++-dev && \
...
```
curl http://localhost:8080/metrics -H "Authorization: Bearer 6976f434-d9c1-11f0-93b8-5254000f64af" -s | jq
```
...
```
curl http://localhost:8080/version -H "Authorization: Bearer 6976f434-d9c1-11f0-93b8-5254000f64af" -s | jq
```
...
to generate your API Key and distribute it to your monitoring agents. Please note that if there is no `API_KEY` environment variable defined in run.sh then the diagnostics APIs will respond without the security check, even if an `Authentication: Bearer <token>` header was sent in the request.

Try calling `/metricsp` for Prometheus, the output is plain text, you do not need `jq` for JSON pretty printing:
```
curl http://localhost:8080/metricsp -H "Authorization: Bearer 6976f434-d9c1-11f0-93b8-5254000f64af"
```
...
| make server_sanitize_thread     | apiserver_sanitizer_thread     | Detects data races
